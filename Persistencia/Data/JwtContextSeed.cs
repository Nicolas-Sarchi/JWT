...
public class JwtContextSeed
{
    public static async Task SeedAsync(JwtContext context, ILoggerFactory loggerFactory)
    {
        try
        {
                    ...
                    using (var csvProductos = new CsvReader(readerProductos, CultureInfo.InvariantCulture))
                    {
                        ...
                        foreach (var item in listadoProductosCsv)
                        {
                            productos.Add(new Producto
                            {
                                ...
                                IdCategoriaFk = item.IdCategoriaFk,
        ...
        {
            var logger = loggerFactory.CreateLogger<JwtContext>();
            logger.LogError(ex.Message);
        }
    }

    public static async Task SeedRolesAsync(JwtContext context, ILoggerFactory loggerFactory)
    {
        try
        {
            if (!context.Rols.Any())
